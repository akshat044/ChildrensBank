	...
	public class BranchDto {
	    ...
	    @Pattern(regexp = "^[A-Z]{4}0[A-Z0-9]{6}$",
	             message = "Invalid IFSC format (Eg: SBIN0XXXXXX)")
	    private String ifscCode;
	
	    @NotBlank(message = "Branch name is required")
	    @Size(min = 2, max = 50, message = "Branch name must be between 2 to 50 characters")
	    @Pattern(regexp = "^[A-Za-z]+(?:[\\s.'-][A-Za-z]+)*$",
	             message = "Branch name may contain only letters and basic punctuation")
	    ...
	    @Pattern(regexp = "^[A-Za-z]+(?:[\\s.'-][A-Za-z]+)*$",
	             message = "City may contain only letters and basic punctuation")
	    private String branchCity;
	
	    @NotBlank(message = "Branch pincode is required")
	    @Pattern(regexp = "^[1-9][0-9]{5}$",
	             message = "Branch pincode must be valid 6-digit Indian PIN")
	    private String branchPincode;
	
	    @NotBlank(message = "Branch status is required")
	    @Pattern(regexp = "^(ACTIVE|CLOSED|TEMPORARY|MAINTENANCE)$",
	             message = "Branch status must be ACTIVE / CLOSED / TEMPORARY / MAINTENANCE")
